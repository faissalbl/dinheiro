...
class PagamentoDAOTest(GenericTest):
    ...
    def testFind(self):
        ...
        despesaFilter = self.getDespesaTempFilterModel1()
        month = datetime.strptime(despesaFilter.despesa.month, '%Y-%m-%d').date()
        pmonth = None
        for i in range(3):
        ...
        despesaFilter = self.getDespesaTempFilterModel2()
        month = datetime.strptime(despesaFilter.despesa.month, '%Y-%m-%d').date()
        pmonth = None
        for i in range(3):
            pmonth = date(month.year, month.month + i, month.day)
    ...
    def createAndAddPagamentos(self, despesaTemp):
        ...
        for i in range(3):
            month = date(month.year, month.month + i, month.day)
            p = Pagamento(despesaTemp = despesaTemp, val = i, paid = 0, month = month)
            print('adding pagamento {}'.format(p))
